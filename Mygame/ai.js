function AIthink(player, move){ // AI will think for a player.
    ...
    function cval(){  // evaluate the board
        let cval_value = 0;
        let val = function(XO){
            let mArray = []; //Match [^1][01][01][01][01][01]0
            let value = 0;
            let count = 0 // count the number of pieces
        ...
        let valStr = ''; //1:X, 2: O, 0: Empty or Out of Board, 3: concat two line
        ...
        // dia from top
        //1:X, 2: O, 0: Empty or Out of Board, 3: concat two line
        for (let k = 0; k < currGame.noOfCol; k++) {
            valStr += '0';
            i=0; j=k;
